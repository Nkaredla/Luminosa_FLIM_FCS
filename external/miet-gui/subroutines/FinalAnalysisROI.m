function [val_mean,val_stdDev,field_val] = FinalAnalysisROI(A,nBin,B)
...
if nargin>2 && ~isempty(B) % do we want to reject dim pixels?
    flag=true;
    if size(B)~=size(A)
...
if size(A,1)>1 && size(A,2)>1 % open image, let user select ROI, collect values from pixels
    ...
    if flag
        ...
        prompt='Please enter a threshold value for rejecting dim pixels (default 50): ';
        threshold=input(prompt);
        if isempty(threshold); threshold=50; end
        pixels = pixels & (B>threshold); % delete all dim pixels
    end
