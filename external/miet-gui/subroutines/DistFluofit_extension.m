function [cx, tau, offset, csh, z, t, err] = DistFluofit_extension(irf, y, p, dt, shift, flag, bild, N, taumax)
...
tau = (1/dt)./exp((0:N)/N*log(taumax/dt)); % distribution of decay times
...
if bild
    ...
    hold all
    semilogy(t,z);
%     semilogy(t,1e3*irf,'k','linewidth',2);
    hold off
...
if flag>0
    ...
    if isempty(t1)|| isempty(t2)
        [cx, tau, offset, csh, z, t, err] = DistFluofit_extension(irf, y, p, dt, shift, flag, bild, N, 2.*taumax);
    else
        if t1(1)>t2(1)
            t2(1)=[];
        end
