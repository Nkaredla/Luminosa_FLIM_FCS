...
zz = z; zz(~isfinite(z)) = 0;                   % zz contains image without invalid numbers
mx = sum(zz.^2*tx)/sum(sum(zz.^2));             % average x-value
my = sum(ty*zz.^2)/sum(sum(zz.^2));             % average y-value
wx = sqrt(sum(zz*(tx-mx).^2)/sum(sum(zz)))/2;   % sigma_x/2
wy = sqrt(sum((ty-my).^2*zz)/sum(sum(zz)))/2;   % sigma_y/2
p = Simplex('Gauss',[mx wx],[-inf 0],[],[],[],tx,sum(z),1,[],pic);
mx = p(1); wx = 2*p(2);                         % mx=mu_x, wx=2*sigma_x
p = Simplex('Gauss',[my wy],[-inf 0],[],[],[],ty',sum(z,2),1,[],pic);
my = p(1); wy = 2*p(2);                         % my=mu_y, wy=2*sigma_y
