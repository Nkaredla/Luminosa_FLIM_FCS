function [field,imm,imm_c, xc, yc, mask, al_res, be_res, theta_mol, phi_mol,err]=pattern_detect(pixel,tag,pattern, NA, n0, n, n1, d0, d, d1, lamex, focpos, flag, flagauto, pic)
...
if nargin<3 || isempty(pattern)
    ...
    if strcmpi(flag,'lifetime')|| strcmpi(flag,'MIET') || strcmpi(flag,'pattern match')
        if strcmpi(pattern,'linear')
            ...
            [err, bim, cim, sim, xc, yc, bc, cc, sc, len, imm, imm_c] = FindPatternSep(tag,mask,mask>0,[],[],0.5); 
            if pic==1
                figure
                mim(cat(3,tag,imm_c));
    ...
    elseif strcmpi(flag,'random')
            al_res=NaN;
            be_res=NaN;
           [err, bim, cim, sim, xc, yc, bc, cc, sc, len, imm, imm_c, mask] = radialpatterns_random(tag, pattern, pixel, NA, n0, n, n1, d0, d, d1, lamex, focpos, pic);
    end
    ...
    if numel(xc)>0
        for i=1:numel(sc)
              ind = imm(:,:,i)>max(max(imm(:,:,i))).*0.04; % index of pixels belonging to i-th pattern (only with at least 4% of the intensity of the maximum of the pattern)
...
%              bright(i)=max(max(field(:,:,i)))./bg;
              ...
              if bright(i)<1.5 || bright(i)==1.5
                 field(:,:,i)=0;
                  ...
                  ind=field(:,:,i)>0.15.*(max(max(field(:,:,i))));
                  field(:,:,i)=ind.*1;
              end
