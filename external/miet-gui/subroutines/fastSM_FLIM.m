function[head, FLIM, INT, SM]= fastSM_FLIM(name, IRF, pattern, NA, n0, n, n1, d0, d, d1, lamex, focpos, flagint, pic)
...
tailfit = 0; flagfit = 1; route = 0; % DistTail fitting
cutoff = 0.5; % 0.5 ns cutoff for tailfitting or for intensity image filtering

disp('Reading image...')

if strcmpi(name(end-3:end),'.ht3')
...
elseif strcmpi(name(end-3:end),'.ptu');
    if exist([name(1:end-4),'_FLIM_data.mat'],'file')
        load([name(1:end-4),'_FLIM_data.mat'],'head','im_tcspc','im_chan','im_line','im_col');
...
%         NCounts=head.TTResult_NumberOfRecords;
        acqtime = head.MeasDesc_AcquisitionTime;
        head.Resolution = Resolution./1e-9;
        head.SyncRate   = head.TTResult_SyncRate;
        head.Records    = head.TTResult_NumberOfRecords;
...
%         NCounts=head.TTResult_NumberOfRecords;
        ...
        head.ImgHdr.X0 = head.ImgHdr_X0;
        head.ImgHdr.Y0 = head.ImgHdr_Y0;
%         save([name(1:end-4),'_FLIM_data.mat'],'head','im_tcspc','im_chan','im_line','im_col');
    end
...
for ch = 1:maxch_n
    ...
    if flagint
        ...
        tag(:,:,ch) = sum(tcspc_pix(:,:,t0:t0+ceil(10/Resolution),ch));
    else
...
if flagfit
    ...
    if route
        ...
        tend = t0+ceil(8./Resolution); % 10 ns maximum cutoff; adjust as per requirement
        comp = 0;
        life_mat=cell(size(field,3),1);
        life_matav=zeros(size(field,3),1);
        disp('Fitting Lifetimes...')
        for j=1:size(life_mat,1)
            if nargin<2 || isempty(IRF)
                ...
                [A, tau,~, ~, z, ~] = DistTailfit(tcspc_sm(t0(j):tend(j)), Resolution,1,[],[],4); %intlife, [zeros(length(intlife),1);5.*ones(length(intlife),1)]);
            else
                
                [A, tau,~, ~, z, ~] = DistTailfit(tcspc_sm(t0(j):tend(j),j), Resolution,1,[],[],4);
            end
            life_mat{j}=1./tau; % check this!
            Amp{j}=A;
            fitz{j}=z;
