...
for k=1:length(varargin)
    ...
    elseif isnumeric(varargin{k}) && isempty(frmprsc)
        frmprsc = varargin{k};
        n = size(frmprsc,3);
        C = gray(255)*255;  % Get the figure's colormap.
        L = size(C,1);
        reverseStr = [];

        for i = 1:n
            msg = sprintf('Converting to movie frame %i/%i ..\n', i, n);
            fprintf([reverseStr, msg]);
            reverseStr = repmat(sprintf('\b'), 1, numel(msg));% used for one-line printing
            
            G = double(frmprsc(:,:,i));
            % Scale the matrix to the range of the map.
            Gs = round(interp1(linspace(min(G(:)),max(G(:)),L),1:L,G));
            movieframes(i).cdata = uint8(reshape(C(Gs,:),[size(Gs) 3]));
            movieframes(i).colormap = [];
        end
...
iptsetpref('ImshowInitialMagnification')
