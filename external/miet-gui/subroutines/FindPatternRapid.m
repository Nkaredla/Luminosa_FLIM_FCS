function [err, bim, cim, sim, xc, yc, bc, cc, sc, len, imm, imm_c] = FindPatternRapid(im,mask,sup,bck,sze,tsh,fun,flag)
...
bim  = im;
sim  = 1+0*bim;
im0  = mConv2(im,sup.*bck);
im02 = mConv2(im.^2,sup);

for s = 1:n3
    crs = sum(sum(sup.*bck.*mask(:,:,s)));
    crs = inv([1 crs; crs 1]);
    im1 = mConv2(im,sup.*mask(:,:,s));
