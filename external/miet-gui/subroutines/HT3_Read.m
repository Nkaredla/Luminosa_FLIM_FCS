...
%  to the position cnts(1) before reading the cnts(2) records of data.
%
...
if (nargout==7)||(cnts(3)==0) % am I supposed to read the header?
...
%         end;
        ...
        head.length = ftell(fid); % header length in bytes
    else
...
if cnts(2)>0 % am I supposed to read any data?
    % if I was supposed to skip the header, i.e. the first cnts(3) bytes,
    % set the file position indicator to cnts(3) bytes after the 'beginning
    % of file'
    if cnts(3)>0  
        fseek(fid, (cnts(3)), 'bof');
    end
    % if I am supposed to start at the data point cnts(1), move the
    % file position indicator cnts(1)-1 int-values from the current position
    if cnts(1)>1
        fseek(fid, 4*(cnts(1)-1), 'cof');
    end

    % read cnts(2) unsigned integers from file (or until end of file)
    % T3Record = column vector containing the data, num = numel(T3Record)
    [T3Record num] = fread(fid, cnts(2), 'ubit32'); % all 32 bits:
    
    % At excitation rates of ~MHz, the total number of sync events is much 
    % too large to count them all in a part of an integer. Instead, every 
    % time the sync-number overflows, the first bit of T3Record is set to 1. 
    % By keeping track of the number of such overflows, we know how many 
    % times we have to add 'WRAPAROUND' (i.e. the maximum size of sync) to
    % sync to get the true value.
    WRAPAROUND=1024;
    special = bitand(T3Record,2147483648)>0;        %2147483648 = 1[31 zeros]  
    % detector number (0-63) is coded in bits 2-7
    chan = bitand(bitshift(T3Record,-25),63);       %63 = 111111  
    % tcspc-bin of the event (0-32767) is coded in bits 8-22
    tcspc = bitand(bitshift(T3Record,-10),32767);   % 32767 = [15 ones]
    % number of sync events that preceded this detection event (0-1023) are
    % coded in bits 23-32 
    sync = bitand(T3Record,1023);                   % 1023 = [10 ones]

    ind  = (special==1 & chan==63); % at which events did special overflow? (don't know why we need chan==63)
    tmp  = sync(ind==1); 
    tmp(tmp==0) = 1; % shouldn't sync always equal 0 if there's an overflow..? 
    sync(ind) = tmp;
    sync = sync + WRAPAROUND*cumsum(ind.*sync); % set sync to "true" value (why not simpy cumsum(ind)?)   
    
    sync(ind)    = [];
    tcspc(ind)   = [];
    special(ind) = [];
    chan(ind)    = [];
    % find number of entries after last valid photon (i.e. after last entry with special==0)
