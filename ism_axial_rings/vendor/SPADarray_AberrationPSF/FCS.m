...
%--------------------------------------------------------------------------
% FCS   Compute the FCS autocorrelation (and cross-correlation) model
%       from azimuthal harmonic coefficients of the PSF.
%
% INPUTS (cylindrical/polar coordinates)
%   rho    : [Nr × Nz] radial coordinate grid (µm); each column = one z-plane
%   z      : [1 × Nz]  axial coordinate grid  (µm); [] for 2-D diffusion
%   volx1  : [Nr × Nz × (2M+1)] azimuthal harmonic coefficients of PSF-1
%              index 1         → DC (m=0)
%              index 2..M+1   → cos(m·φ) coefficients,  m = 1..M
%              index M+2..2M+1→ sin(m·φ) coefficients,  m = 1..M
%   voly1  : same size as volx1 (e.g. second polarisation component)
%              set to zeros(size(volx1)) for a single-component PSF
%   volx2,voly2 : optional second PSF for cross-correlation (same format)
%   d      : distance to a reflective boundary along z [µm]; 0 = none
%   velo   : [vx vy] flow velocity vector; [] = pure diffusion
%   dif    : diffusion coefficient [µm² / time-unit]  (default 5e-5)
%   bld    : 1 = live semilog plot during computation; 0 = silent
%
% OUTPUTS
%   modres   : [nTime × 1] normalised autocorrelation G(τ)/G(0)
%              columns 2-4 present only when cross-correlation requested
%   autotime : [nTime × 1] lag-time axis (same time-units as dif)
%
% MATHEMATICAL BASIS
%   The 3-D FCS autocorrelation is:
%       G(τ) = ∫∫ h(r)·G_diff(r,r',τ)·h(r') d³r d³r'
%   expressed in azimuthal harmonics. Each harmonic m decouples:
%   - Radial propagation: I_m Bessel kernel (modified Bessel function)
%   - Axial  propagation: erf-based Gaussian Green's function
%   The final integral over m is weighted [2,1,1,...,1] to account for
%   the azimuthal integration of cos² (m=0 → 2π, m>0 → π).
%--------------------------------------------------------------------------
