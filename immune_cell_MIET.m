...
% one-, two-, and three-exponential Bayesian models in membrane pixels. The
% SLB amplitude may be free, hard-constrained, or spatially regularized from
% a smooth outside-SLB calibration extrapolated underneath the cell.
%
...
%   slbAmplitudeMode      'free', 'regularized', or 'fixed'; an empty value
%                         derives legacy behaviour from fixSlbAmplitude
%   fixSlbAmplitude       legacy switch: false -> free, true -> fixed
%   slbAmplitudeScale     multiplier for that outside calibration when the
%                         amplitude constraint is enabled (default 1)
%   slbAmplitudePriorRelativeStdFloor minimum relative prior width (0.10)
%   slbAmplitudePriorRelativeStdCeiling maximum relative prior width (0.50)
%   maxSlbClippedFraction warning threshold when observed N is below the
%                         usable SLB-amplitude prior support (default 0.10)
%   componentMaps         posterior/photon gates for sorted tau2/tau3 layers
%   maxMembraneStates     highest free membrane-state count compared per
%                         pixel: 2 for the full three-model comparison, 1
%                         for fixed SLB + one free membrane lifetime (2)
...
%   spatialBinning4x4     the corresponding 4x4, one-pixel-step analysis
    ...
    evaluateThirdComponent = cfg.maxMembraneStates >= 2;
    if evaluateThirdComponent
        modelSetLabel = 'SLB, biexponential, triexponential';
    else
        modelSetLabel = 'SLB, biexponential';
    end
    ...
    if cfg.fixSlbAmplitude
        slbAmplitude = estimateSlbAmplitudeCalibration(slbCurve, slbFitCounts, ...
            slbCoefficients, intensity, masks.slbReference, ...
            masks.outsideSlbGeometry, cfg);
        slbAmplitude.constraintApplied = true;
        fprintf(['immune_cell_MIET: %s SLB amplitude prior %.3g +/- %.3g ' ...
            'photons/native pixel from a smooth outside-SLB surface ' ...
            '(%d calibration pixels).\n'], cfg.slbAmplitudeMode, ...
            slbAmplitude.signalPhotonsPerPixel, ...
            slbAmplitude.dispersionPhotonsPerPixel, ...
            slbAmplitude.amplitudePopulationPixelCount);
    else
        slbAmplitude = unconstrainedSlbAmplitudeMetadata();
        fprintf(['immune_cell_MIET: fixing the outside-SLB lifetime only; ' ...
            'SLB and membrane amplitudes remain free per pixel.\n']);
    end
    ...
    bayesOpts.maxMembraneStates = cfg.maxMembraneStates;
    if cfg.fixSlbAmplitude
        bayesOpts.fixedSlbPhotonCount = ...
            double(slbAmplitude.priorMeanPhotonCountMapNative( ...
            membranePixelIndices));
        bayesOpts.fixedSlbPhotonCountStd = ...
            double(slbAmplitude.priorStdPhotonCountMapNative( ...
            membranePixelIndices));
    else
    ...
    clear membraneCube
    [bayesianStatus, bayesianValidForInterpretation, slbAmplitude] = ...
        assessSlbCountClipping(bayesCompact, slbAmplitude, cfg, ...
        'immune_cell_MIET:SlbAmplitudePriorClipping', 'native');
    ...
    binnedComponentPhotonFigureHandle = [];
    ...
    if cfg.spatialBinning.enabled
        ...
        binnedBayesOpts.maxMembraneStates = cfg.maxMembraneStates;
        if cfg.fixSlbAmplitude
            binnedBayesOpts.fixedSlbPhotonCount = ...
                binnedSlbAmplitude.priorMeanPhotonCount;
            binnedBayesOpts.fixedSlbPhotonCountStd = ...
                binnedSlbAmplitude.priorStdPhotonCount;
        else
            binnedBayesOpts.fixedSlbPhotonCount = [];
            binnedBayesOpts.fixedSlbPhotonCountStd = [];
        end
        fprintf(['immune_cell_MIET: running the 2x2/step-1 fixed-SLB ' ...
            'Bayesian comparison (%s).\n'], modelSetLabel);
        ...
        spatialBinningResult.bayesian = struct( ...
            ...
            'tcspcMeta', binnedTcspcMeta, 'candidateOrders', ...
                1:(cfg.maxMembraneStates + 1), ...
    ...
    binned4x4ComponentPhotonFigureHandle = [];
    ...
    if cfg.spatialBinning4x4.enabled
        ...
        binned4x4BayesOpts.maxMembraneStates = cfg.maxMembraneStates;
        if cfg.fixSlbAmplitude
            binned4x4BayesOpts.fixedSlbPhotonCount = ...
                binned4x4SlbAmplitude.priorMeanPhotonCount;
            binned4x4BayesOpts.fixedSlbPhotonCountStd = ...
                binned4x4SlbAmplitude.priorStdPhotonCount;
        else
            binned4x4BayesOpts.fixedSlbPhotonCount = [];
            binned4x4BayesOpts.fixedSlbPhotonCountStd = [];
        end
        ...
        spatialBinning4x4Result.bayesian = struct( ...
            ...
            'tcspcMeta', binned4x4TcspcMeta, 'candidateOrders', ...
                1:(cfg.maxMembraneStates + 1), ...
    ...
    if cfg.showFigure || cfg.saveOutputs
        ...
        if cfg.componentMaps.enabled
            ...
            secondLifetimeFigureHandle = showOrderedLifetimeMap( ...
                ...
                tauSlbNs, slbAmplitude, '', evaluateThirdComponent);
            if evaluateThirdComponent
        ...
        if cfg.spatialBinning.enabled
            ...
            binnedBayesFigureHandle = showBayesSummary( ...
                ...
                ['2x2 sliding TCSPC, one-pixel step; native FLIM ' ...
                 'segmentation reused'], cfg.maxMembraneStates);
            if cfg.componentMaps.enabled
                ...
                if evaluateThirdComponent
                    binnedThirdLifetimeFigureHandle = showOrderedLifetimeMap( ...
                        spatialBinningResult.bayesian.orderedComponents, ...
                        masks, 3, binnedLifetimeLimits, visibility, tauSlbNs, ...
                        spatialBinningResult.bayesian.fixedSlbAmplitude, ...
                        '2x2 sliding TCSPC, one-pixel step', ...
                        evaluateThirdComponent);
                end
                binnedLayeredLifetimeFigureHandle = showOrderedLifetimeLayers( ...
                    ...
                    '2x2 sliding TCSPC, one-pixel step', ...
                    evaluateThirdComponent);
                binnedComponentPhotonFigureHandle = showComponentPhotonMap( ...
                    spatialBinningResult.bayesian.orderedComponents, ...
                    visibility, ...
        ...
        if cfg.spatialBinning4x4.enabled
            ...
            binned4x4BayesFigureHandle = showBayesSummary( ...
                ...
                ['4x4 sliding TCSPC, one-pixel step; native FLIM ' ...
                 'segmentation reused'], cfg.maxMembraneStates);
            if cfg.componentMaps.enabled
                ...
                if evaluateThirdComponent
                    binned4x4ThirdLifetimeFigureHandle = ...
                        showOrderedLifetimeMap( ...
                        spatialBinning4x4Result.bayesian.orderedComponents, ...
                        masks, 3, binned4x4LifetimeLimits, visibility, ...
                        tauSlbNs, ...
                        spatialBinning4x4Result.bayesian.fixedSlbAmplitude, ...
                        '4x4 sliding TCSPC, one-pixel step', ...
                        evaluateThirdComponent);
    ...
    result.analysisSchemaVersion = 8;
    ...
    result.bayesian = struct('status', bayesianStatus, ...
        ...
        'tcspcMeta', compactMeta, 'candidateOrders', ...
            1:(cfg.maxMembraneStates + 1));
    ...
    if cfg.saveOutputs
        ...
        if ~isempty(componentPhotonFigureHandle)
            exportgraphics(componentPhotonFigureHandle, ...
                componentPhotonPngFile, 'Resolution', 300);
        end
        if ~isempty(binnedComponentPhotonFigureHandle)
            exportgraphics(binnedComponentPhotonFigureHandle, ...
                binnedComponentPhotonPngFile, 'Resolution', 300);
        end
        if ~isempty(binned4x4ComponentPhotonFigureHandle)
        ...
        expectedFigures = {pngFile, bayesPngFile};
        if cfg.componentMaps.enabled
            expectedFigures = [expectedFigures, ...
                {secondLifetimePngFile, layeredLifetimePngFile, ...
                 componentPhotonPngFile, componentLifetimePngFile}];
            if evaluateThirdComponent
                expectedFigures = [expectedFigures, {thirdLifetimePngFile}];
            end
        end
        if cfg.spatialBinning.enabled
            expectedFigures = [expectedFigures, ...
                {binnedMeanPngFile, binnedBayesPngFile}];
            if cfg.componentMaps.enabled
                expectedFigures = [expectedFigures, ...
                    {binnedSecondLifetimePngFile, ...
                     binnedLayeredLifetimePngFile, ...
                     binnedComponentPhotonPngFile, ...
                     binnedComponentLifetimePngFile}];
                if evaluateThirdComponent
                    expectedFigures = [expectedFigures, ...
                        {binnedThirdLifetimePngFile}];
        ...
        if cfg.spatialBinning4x4.enabled
            expectedFigures = [expectedFigures, ...
                {binned4x4MeanPngFile, binned4x4BayesPngFile}];
            if cfg.componentMaps.enabled
                expectedFigures = [expectedFigures, ...
                    {binned4x4SecondLifetimePngFile, ...
                     binned4x4LayeredLifetimePngFile, ...
        ...
        for figureIndex = 1:numel(expectedFigures)
            if ~isfile(expectedFigures{figureIndex})
                error('immune_cell_MIET:MissingFigure', ...
                    'Expected figure was not written: %s', ...
                    expectedFigures{figureIndex});
            end
        end
        fprintf(['immune_cell_MIET: verified %d saved analysis figures ' ...
            '(native%s%s).\n'], numel(expectedFigures), ...
            enabledFigureLabel(cfg.spatialBinning.enabled, ', 2x2'), ...
            enabledFigureLabel(cfg.spatialBinning4x4.enabled, ', 4x4'));
        if tcspcPixSaved
...
function label = enabledFigureLabel(enabled, label)
    if ~enabled
        label = '';
    end
end
...
function cfg = fillDefaults(cfg)
    amplitudeModeWasSupplied = isfield(cfg, 'slbAmplitudeMode') && ...
        ~isempty(cfg.slbAmplitudeMode);
    ...
    defaults = struct('excitationPulseIndex', 2, 'excitationNm', 640, ...
        ...
        'slbAmplitudeMode', '', 'fixSlbAmplitude', true, ...
        'slbAmplitudeScale', 1, ...
        'slbAmplitudePriorRelativeStdFloor', 0.10, ...
        'slbAmplitudePriorRelativeStdCeiling', 0.50, ...
        'slbAmplitudeSurfaceClipQuantiles', [0.05 0.95], ...
        'maxSlbClippedFraction', 0.10, ...
        'bayesMinPhotons', 10, 'maxMembraneStates', 2, ...
        'ism', struct(), 'segmentation', struct(), ...
    ...
    if ~amplitudeModeWasSupplied
        if cfg.fixSlbAmplitude
            cfg.slbAmplitudeMode = 'fixed';
        else
            cfg.slbAmplitudeMode = 'free';
        end
    end
    cfg.slbAmplitudeMode = validatestring(lower(char(cfg.slbAmplitudeMode)), ...
        {'free','regularized','fixed'}, mfilename, 'cfg.slbAmplitudeMode');
    cfg.fixSlbAmplitude = ~strcmp(cfg.slbAmplitudeMode, 'free');
    ...
    validateattributes(cfg.slbAmplitudePriorRelativeStdFloor, {'numeric'}, ...
        {'real','finite','scalar','positive','<',1});
    validateattributes(cfg.slbAmplitudePriorRelativeStdCeiling, {'numeric'}, ...
        {'real','finite','scalar','positive','<',1});
    if cfg.slbAmplitudePriorRelativeStdCeiling < ...
            cfg.slbAmplitudePriorRelativeStdFloor
        error('immune_cell_MIET:SlbAmplitudePriorWidth', ...
            ['slbAmplitudePriorRelativeStdCeiling must be at least the ' ...
             'corresponding floor.']);
    end
    validateattributes(cfg.slbAmplitudeSurfaceClipQuantiles, {'numeric'}, ...
        {'real','finite','vector','numel',2,'>=',0,'<=',1});
    cfg.slbAmplitudeSurfaceClipQuantiles = ...
        sort(double(cfg.slbAmplitudeSurfaceClipQuantiles(:))).';
    if cfg.slbAmplitudeSurfaceClipQuantiles(2) <= ...
            cfg.slbAmplitudeSurfaceClipQuantiles(1)
        error('immune_cell_MIET:SlbAmplitudeClipQuantiles', ...
            'The two SLB surface clipping quantiles must be distinct.');
    end
    if strcmp(cfg.slbAmplitudeMode, 'regularized')
        if ~isfield(cfg.bayes, 'slbCountPriorNodes') || ...
                isempty(cfg.bayes.slbCountPriorNodes)
            cfg.bayes.slbCountPriorNodes = 5;
        end
        if ~isfield(cfg.bayes, 'slbCountRelTol') || ...
                isempty(cfg.bayes.slbCountRelTol)
            cfg.bayes.slbCountRelTol = 0.0025;
        end
        if cfg.bayes.slbCountPriorNodes <= 0
            error('immune_cell_MIET:RegularizedSlbPriorNodes', ...
                ['slbAmplitudeMode=''regularized'' requires a positive odd ' ...
                 'cfg.bayes.slbCountPriorNodes value.']);
        end
    elseif strcmp(cfg.slbAmplitudeMode, 'fixed')
        % The explicit hard mode is exact even when a reused bayes struct
        % contains prior-marginalisation settings from another analysis.
        cfg.bayes.slbCountPriorNodes = 0;
    end
    validateattributes(cfg.maxSlbClippedFraction, {'numeric'}, ...
        {'real','finite','scalar','>=',0,'<=',1});
    validateattributes(cfg.maxMembraneStates, {'numeric'}, ...
        {'real','finite','scalar','integer','>=',1,'<=',2});
    cfg.maxMembraneStates = double(cfg.maxMembraneStates);
...
function amplitude = unconstrainedSlbAmplitudeMetadata()
    amplitude = struct( ...
        'constraintApplied', false, ...
        'mode', 'free', ...
        'method', 'not estimated because the SLB amplitude is free per pixel', ...
        'scale', NaN, ...
        'signalPhotonsPerPixel', NaN, ...
        'dispersionPhotonsPerPixel', NaN, ...
        'poissonSigmaPhotonsPerPixel', NaN, ...
        'robustSigmaPhotonsPerPixel', NaN, ...
        'relativeDispersion', NaN, ...
        'bayesianClippedPixelCount', 0, ...
        'bayesianClippedPixelFraction', 0);
end

function amplitude = estimateSlbAmplitudeCalibration(slbCurve, slbFitCounts, ...
        coefficients, intensity, referenceMask, amplitudeMask, cfg)
    ...
    % The pooled lifetime-stable reference determines the SLB signal fraction.
    % Apply it to all geometric outside-SLB pixels, robustly fit a smooth
    % quadratic illumination/detection surface there, and evaluate that surface
    % over the full field. This is the calibration extrapolated beneath the cell.
    outsideCounts = max(double(intensity(amplitudeMask)), 0);
    proxySignalCounts = scale * signalFraction * outsideCounts;
    [surface, surfaceCoefficients, surfaceResidualLogSigma, surfaceClipBounds] = ...
        robustSpatialSlbSurface(proxySignalCounts, amplitudeMask, ...
        size(intensity), cfg.slbAmplitudeSurfaceClipQuantiles);
    signalPhotonsPerPixel = mean(surface(amplitudeMask));
    ...
    surfaceRelativeSigma = sqrt(max(expm1(surfaceResidualLogSigma ^ 2), 0));
    relativePriorStd = min(max(surfaceRelativeSigma, ...
        cfg.slbAmplitudePriorRelativeStdFloor), ...
        cfg.slbAmplitudePriorRelativeStdCeiling);
    priorStdMap = sqrt(max(surface, 1) + (relativePriorStd * surface) .^ 2);
    dispersion = mean(priorStdMap(amplitudeMask));

    amplitude = struct();
    amplitude.constraintApplied = true;
    amplitude.mode = cfg.slbAmplitudeMode;
    amplitude.method = ['pooled outside-SLB IRF fit for signal fraction; ' ...
        'robust quadratic log-amplitude surface fitted on geometric outside-SLB ' ...
        'pixels and extrapolated beneath the cell'];
    ...
    amplitude.spatialPriorRelativeStd = relativePriorStd;
    amplitude.surfaceResidualLogSigma = surfaceResidualLogSigma;
    amplitude.surfaceCoefficients = surfaceCoefficients(:).';
    amplitude.surfaceBasis = ...
        '1, normalized row, normalized column, row^2, row*column, column^2';
    amplitude.surfaceClipBoundsPhotonsPerPixel = surfaceClipBounds;
    amplitude.surfaceClipQuantiles = cfg.slbAmplitudeSurfaceClipQuantiles;
    amplitude.priorMeanPhotonCountMapNative = single(surface);
    amplitude.priorStdPhotonCountMapNative = single(priorStdMap);
    amplitude.dispersionUsedForInference = ...
        strcmp(cfg.slbAmplitudeMode, 'regularized');
...
function [surface, coefficients, residualSigma, clipBounds] = ...
        robustSpatialSlbSurface(proxyCounts, mask, imageSize, clipQuantiles)
    [rowGrid, columnGrid] = ndgrid(1:imageSize(1), 1:imageSize(2));
    rowCoordinate = (double(rowGrid) - (imageSize(1) + 1) / 2) / ...
        max((imageSize(1) - 1) / 2, 1);
    columnCoordinate = (double(columnGrid) - (imageSize(2) + 1) / 2) / ...
        max((imageSize(2) - 1) / 2, 1);
    x = rowCoordinate(mask);
    y = columnCoordinate(mask);
    design = [ones(nnz(mask), 1), x, y, x .^ 2, x .* y, y .^ 2];
    response = log(max(double(proxyCounts(:)), 0.5));
    if size(design, 1) < 12 || rank(design) < size(design, 2)
        coefficients = [median(response); zeros(5, 1)];
        residual = response - coefficients(1);
        residualSigma = 1.4826 * median(abs(residual - median(residual)));
    else
        ...
        for iteration = 1:12
            ...
            residualSigma = 1.4826 * median(abs(residual - residualMedian));
            if ~(isfinite(residualSigma) && residualSigma > eps)
                break;
            end
            standardized = abs(residual - residualMedian) / ...
                (1.345 * residualSigma);
            weight = ones(size(standardized));
            outsideHuberCore = standardized > 1;
            weight(outsideHuberCore) = 1 ./ standardized(outsideHuberCore);
            weightedDesign = design .* sqrt(weight);
            updated = weightedDesign \ (response .* sqrt(weight));
            if norm(updated - coefficients) <= ...
                    1e-7 * max(norm(coefficients), 1)
    ...
    if ~(isfinite(residualSigma) && residualSigma > 0)
        residualSigma = 0;
    end

    fullDesign = [ones(numel(rowGrid), 1), rowCoordinate(:), ...
        columnCoordinate(:), rowCoordinate(:) .^ 2, ...
        rowCoordinate(:) .* columnCoordinate(:), columnCoordinate(:) .^ 2];
    surface = reshape(exp(fullDesign * coefficients), imageSize);
    observedMedian = median(proxyCounts);
    predictedMedian = median(surface(mask));
    if isfinite(observedMedian) && observedMedian > 0 && ...
            isfinite(predictedMedian) && predictedMedian > 0
        surface = surface * observedMedian / predictedMedian;
    end
    clipBounds = finitePercentiles(proxyCounts, clipQuantiles);
    clipBounds(1) = max(clipBounds(1), eps);
    if ~(clipBounds(2) > clipBounds(1))
        clipBounds = [max(observedMedian * 0.5, eps), ...
            max(observedMedian * 1.5, 2 * eps)];
    end
    surface = min(max(surface, clipBounds(1)), clipBounds(2));
end

function amplitude = slidingSlbAmplitude(nativeAmplitude, windowSize, step, ...
        pixelIndices, imageSize)
    area = prod(double(windowSize));
    amplitude = rmfield(nativeAmplitude, ...
        {'priorMeanPhotonCountMapNative','priorStdPhotonCountMapNative'});
    ...
    meanMap = completeWindowSum( ...
        double(nativeAmplitude.priorMeanPhotonCountMapNative), windowSize, ...
        imageSize);
    priorMean = meanMap(pixelIndices);
    relativeStd = nativeAmplitude.spatialPriorRelativeStd;
    priorStd = sqrt(max(priorMean, 1) + (relativeStd * priorMean) .^ 2);
    amplitude.priorMeanPhotonCount = single(priorMean(:));
    amplitude.priorStdPhotonCount = single(priorStd(:));
    amplitude.signalPhotonsPerPixel = mean(priorMean);
    amplitude.dispersionPhotonsPerPixel = mean(priorStd);
    ...
    amplitude.method = [nativeAmplitude.method '; each window prior mean is ' ...
        'the exact sum of its native-pixel calibration surface'];
    ...
    amplitude.dispersionUsedForInference = ...
        strcmp(nativeAmplitude.mode, 'regularized');
    amplitude.bayesianClippedPixelCount = 0;
    amplitude.bayesianClippedPixelFraction = 0;
end

function summed = completeWindowSum(map, windowSize, imageSize)
    valid = conv2(double(map), ones(double(windowSize)), 'valid');
    summed = nan(imageSize);
    summed(1:size(valid, 1), 1:size(valid, 2)) = valid;
end
...
function [status, validForInterpretation, amplitude] = ...
    ...
    status = 'invalid_slb_amplitude_prior_clipping';
    validForInterpretation = false;
    warning(warningIdentifier, ...
        ['The observed total was below the usable support of the outside-derived ' ...
         'SLB amplitude calibration in %.1f%% of fitted %s pixels. Maps are ' ...
         'saved for diagnosis but are marked invalid.'], ...
        100 * amplitude.bayesianClippedPixelFraction, ...
        analysisLabel);
end
...
function metadata = bayesianModelSelectionMetadata(bayesCompact, componentCfg)
    ...
    if isfield(bayesCompact, 'maxMembraneStates')
        metadata.maxMembraneStates = bayesCompact.maxMembraneStates;
        metadata.evaluatedModels = bayesCompact.evaluatedModels;
    else
        metadata.maxMembraneStates = 2;
        metadata.evaluatedModels = true(1, 3);
    end
...
function h = showBayesSummary(meanArrival, intensity, masks, bayes, tauSlb, ...
        visibility, analysisLabel, maxMembraneStates)
    if nargin < 7
        analysisLabel = '';
    end
    if nargin < 8
    ...
    if maxMembraneStates >= 2
        ...
        showImage(ax, bayes.selected.membraneLifetime1Ns, turbo(256), ...
            'Selected longer membrane lifetime (ns)');
    end
...
function h = showComponentPhotonMap(layers, visibility, analysisLabel, ...
        showThirdComponent)
%SHOWCOMPONENTPHOTONMAP Absolute detected photons per lifetime component.
%
% One panel per component. The value is the pixel photon total times that
% component's posterior photon fraction, so it is an expected detected photon
% count, not a fitted amplitude. Each panel carries its own colour scale
% because the fixed-SLB component usually holds far more photons than the
% membrane components. Panels 2 and 3 use the same presence and
% expected-photon gates as the sorted-lifetime figures, so a pixel is shown
% only where that component is actually identified.
    if nargin < 3
        analysisLabel = '';
    end
    if nargin < 4
        showThirdComponent = true;
    end

    photons = layers.expectedPhotonCount;
    panels = struct( ...
        'data', {photons.firstComponent, photons.secondComponent, ...
                 photons.thirdComponent}, ...
        'mask', {layers.masks.cell, layers.masks.secondDisplay, ...
                 layers.masks.thirdDisplay}, ...
        'name', {'Component 1 (SLB; fixed lifetime)', 'Component 2', ...
                 'Component 3 (longest)'});
    if ~showThirdComponent
        panels = panels(1:2);
    end

    h = figure('Name', 'immune_cell_MIET: component photon counts', ...
        'Color', 'w', 'Visible', visibility, 'Position', [80 80 1500 700]);
    layout = tiledlayout(h, 1, numel(panels), 'Padding', 'compact', ...
        'TileSpacing', 'compact');

    for index = 1:numel(panels)
        data = double(panels(index).data);
        mask = logical(panels(index).mask) & isfinite(data) & data > 0;
        ax = nexttile(layout);
        ...
        colormap(ax, turbo(256));
        % Robust upper limit: a few very bright pixels would otherwise flatten
        % the whole panel.
        values = data(mask);
        if isempty(values)
            clim(ax, [0 1]);
            ...
            % Robust range on BOTH ends. Anchoring every panel at zero can
            % hide spatial structure, especially when a component occupies a
            % narrow count range. A 1st-99th percentile window keeps fixed-
            % count and free-amplitude analyses readable.
            ...
            if spread <= 1e-6 * max(abs(middle), eps)
                % Genuinely uniform. Centre the constant in the colour map
                % and say so instead of stretching over numerical noise and
                % implying spatial structure that does not exist.
        ...
        title(ax, titleText, 'FontSize', 9);
    end

    headline = 'Absolute photons per lifetime component';
    if ~isempty(analysisLabel)
...
function h = showComponentLifetimeMap(layers, limits, tauSlbNs, ...
        visibility, analysisLabel, showThirdComponent)
...
function h = showOrderedLifetimeMap(layers, ~, component, limits, ...
        visibility, tauSlbNs, slbAmplitude, analysisLabel, showThirdComponent)
    if nargin < 8
        analysisLabel = '';
    end
    if nargin < 9
    ...
    if component == 2
        ...
        if showThirdComponent
            ...
            probabilityLabel = 'P(SLB + one membrane state)';
        end
    ...
    subtitleText = slbConstraintSubtitle(tauSlbNs, slbAmplitude);
    if ~isempty(analysisLabel)
...
function h = showOrderedLifetimeLayers(layers, masks, limits, visibility, ...
        tauSlbNs, slbAmplitude, analysisLabel, showThirdComponent)
    if nargin < 7
        analysisLabel = '';
    end
    if nargin < 8
    ...
    if showThirdComponent && any(layers.masks.thirdDisplay(:))
    ...
    colourBar.Label.String = 'Lifetime [ns]';
    if showThirdComponent
        title(ax, sprintf(['Ordered lifetime layers: cyan = component 2 ' ...
            'support; magenta/white = component 3 contours | %d / %d pixels'], ...
            layers.summary.secondDisplayPixelCount, ...
            layers.summary.thirdDisplayPixelCount));
    else
        title(ax, sprintf(['Membrane lifetime support (cyan) | ' ...
            '%d pixels'], layers.summary.secondDisplayPixelCount));
    end
    subtitleText = slbConstraintSubtitle(tauSlbNs, slbAmplitude);
    if ~isempty(analysisLabel)
        subtitleText = sprintf('%s | %s', analysisLabel, subtitleText);
...
function text = slbConstraintSubtitle(tauSlbNs, slbAmplitude)
    if isfield(slbAmplitude, 'constraintApplied') && ...
            logical(slbAmplitude.constraintApplied)
        if isfield(slbAmplitude, 'mode') && ...
                strcmp(slbAmplitude.mode, 'regularized')
            text = sprintf(['Fixed SLB lifetime %.3g ns; smooth spatial ' ...
                'amplitude prior %.3g +/- %.3g photons/pixel; %.1f%% below ' ...
                'prior support'], tauSlbNs, slbAmplitude.signalPhotonsPerPixel, ...
                slbAmplitude.dispersionPhotonsPerPixel, ...
                100 * slbAmplitude.bayesianClippedPixelFraction);
        else
            text = sprintf(['Fixed SLB: tau %.3g ns, expected amplitude ' ...
                '%.3g photons/pixel; %.1f%% clipped'], tauSlbNs, ...
                slbAmplitude.signalPhotonsPerPixel, ...
                100 * slbAmplitude.bayesianClippedPixelFraction);
        end
    else
        text = sprintf(['Fixed SLB lifetime: tau %.3g ns; SLB and membrane ' ...
            'amplitudes fitted per pixel'], tauSlbNs);
    end
end
