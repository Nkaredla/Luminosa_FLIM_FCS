function out = Calc_mIRF_Global_ExGauss_fast(head, tcspc, tau0, opts)
% Calc_mIRF_Global_ExGauss_fast
% Faster global ex-Gaussian IRF fit.
%
% Speedups mirror Calc_mIRF_Global_GammaShifted_fast.
...
[~, indMax] = max(y);
t0_0 = t(indMax);
sigma0 = max(3*Resolution, 0.08);
tauR0  = max(2*Resolution, 0.04);

p0 = [t0_0; sigma0; tauR0; tau0];
pl = [t0_0 - 1.0; max(Resolution/4, 0.005); 0.001; max(0.05 * tau0, 0.01)];
pu = [t0_0 + 1.0; 1.5; 1.5; max(20 * tau0, 10)];
...
for casc = 1:nCasc
    for sub = 1:nSub
        ...
        if exist('Simplex', 'file') == 2
            ...
            pfit = fminsearch(@(p) boundedErrExGauss(p, pl, pu, t, y), rf, optimset('Display','off','MaxIter',150));
...
function err = boundedErrExGauss(p, pl, pu, t, y)
p = min(max(p(:), pl), pu);
err = TCSPC_Fun_ExGauss(p, t, y, []);
end
