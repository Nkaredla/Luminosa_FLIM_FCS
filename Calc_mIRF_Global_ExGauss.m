function out = Calc_mIRF_Global_ExGauss(head, tcspc, tau0)
% Calc_mIRF_Global_ExGauss
%
% Fit one shared ex-Gaussian IRF on the summed TCSPC decay.
% This is intended as a more stable alternative to estimating an IRF
% from individual pixel decays.
%
% Inputs
% ------
% head : PTU header struct
% tcspc: decay vector, image cube, or any array that can be summed to 1 decay
% tau0 : initial lifetime guesses [ns], e.g. [0.35 1.8 4.2]
%
% Output
% ------
% out.IRF      : fitted normalized IRF vector
% out.pIRF     : [t0 sigma tauR]
% out.tauFit   : fitted lifetimes [ns]
% out.A        : fitted amplitudes (background first)
% out.zz       : fitted basis matrix
...
% Initial shift from peak location
[~, indMax] = max(y);
t0_0 = t(indMax);

% Conservative initial IRF parameters
sigma0 = max(3*Resolution, 0.08);   % ns
tauR0  = max(2*Resolution, 0.04);   % ns

p0 = [t0_0; sigma0; tauR0; tau0];

% Bounds
pl = [t0_0 - 1.0; ...
      max(Resolution/4, 0.005); ...
      0.001; ...
      max(0.05 * tau0, 0.01)];
...
for casc = 1:nCasc
    for sub = 1:nSub
        ...
        % Additive randomization for t0
        rf(1) = p0(1) + (1.0 / casc) * (2*rand - 1);

        % Multiplicative randomization for positive parameters
        rf(2:end) = p0(2:end) .* 2.^(0.9 * (rand(size(p0(2:end))) - 0.5) / casc);

        rf = max(rf, pl);
        rf = min(rf, pu);

        pfit = Simplex('TCSPC_Fun_ExGauss', rf, pl, pu, [], [], t, y, []);
        err  = TCSPC_Fun_ExGauss(pfit, t, y, []);
...
[~, cfit, zz, zfit, IRF] = TCSPC_Fun_ExGauss(bestP, t, y, []);
...
% assume time is the last non-singleton dimension after reduction
sz = size(x);
nT = sz(end);
end
