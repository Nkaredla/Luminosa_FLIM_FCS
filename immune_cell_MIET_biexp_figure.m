...
% fine in both cases.
    ...
    panels = { ...
        maps.tau2Ns,             'free component \tau_2 (ns)',      'jet', []; ...
        maps.photonFraction2,    'photon share of \tau_2',           'jet', [0 1]; ...
        maps.tauMeanNs,          'photon-weighted mean \tau (ns)',   'jet', []; ...
        maps.slbPullSigma,       'SLB prior pull (\sigma)',          'coolwarm', [-2 2]; ...
        maps.reducedDeviance,    'reduced deviance',                 'jet',    []; ...
        log10(maps.intensity + 1), 'log_{10} intensity',             'gray',   []};

    for k = 1:size(panels, 1)
        ax = nexttile(layout);
        % Fitted everywhere, drawn inside the cell only - see applyDisplayMask.
        data = applyDisplayMask(panels{k, 1}, out);
