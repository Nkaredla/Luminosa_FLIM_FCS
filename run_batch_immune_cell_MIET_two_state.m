%RUN_BATCH_IMMUNE_CELL_MIET_TWO_STATE Two-component batch immune-cell MIET.
%
% Same acquisition selection, segmentation, fixed-SLB calibration and
% sliding-window stages as RUN_BATCH_IMMUNE_CELL_MIET, but each pixel is
% compared under two models only:
%
%   M1  fixed outside-SLB lifetime alone
%   M2  fixed outside-SLB lifetime + one free membrane lifetime
%
% The triexponential model M3 is neither built nor evaluated, so no third
% component can appear anywhere in the results. Its posterior probability is
% exactly zero by construction, its sorted component-3 layer stays empty, and
% no third-component figure is written. Skipping the M3 grid is also where
% nearly all of this pipeline's cost lives, so the run is much faster and much
% lighter on memory than the three-model one.
%
% Results land beside each PTU in
%   <acquisition>\immune_cell_MIET_two_state\<runName>
% and the batch summary in
%   <dataRoot>\immune_cell_MIET_batch\<runName>
% so nothing here can overwrite the three-model results.
%
% See also RUN_BATCH_IMMUNE_CELL_MIET, BATCH_IMMUNE_CELL_MIET,
% IMMUNE_CELL_MIET, FLIM_BAYES_FIXED_SLB.
...
% A results folder of its own. The three-model run writes
% immune_cell_MIET_grid48; keeping these apart means either can be rerun
% without touching the other.
cfg.resultsFolderName = 'immune_cell_MIET_two_state';
...
pipelineCfg = struct();
% The whole point of this runner. 1 = fixed SLB + one free membrane lifetime;
% 2 would restore the full three-model comparison. The run manifest records
% this value, so a results folder can always be traced back to the model set
% that produced it.
pipelineCfg.maxMembraneStates = 1;
...
% Only the first entry of each pair does anything here: it gates the single
% free membrane lifetime. The second entry gated component 3 and is kept so the
% saved config keeps the same shape as the three-model run.
